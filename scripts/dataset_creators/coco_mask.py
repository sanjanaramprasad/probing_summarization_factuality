...
class Coco:
    ...
    def mask_document(self, source_doc, 
        ...
        # print(masked_doc, tokenized_doc_whitespaces)
        # print(tokenized_doc)
        masked_doc = [tok + tokenized_doc_whitespaces[tok_idx] if tok.strip() else tok for tok_idx, tok in enumerate(masked_doc)]
        #### because falcon has no unknown tokens so the masked tokens are just spaces
        masked_doc = [w for w in ''.join(masked_doc).split(' ') if w.strip()]
        masked_doc = ' '.join(masked_doc)
        # if 0 in mask_matrix:
        #     # print(mask_matrix, tokenized_doc, masked_doc)
        #     print(masked_doc)
