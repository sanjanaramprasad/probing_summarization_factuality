...
def JSD(current_layer, final_layer):
    # print(current_layer.shape, final_layer.shape)
    #### Calculate the softmax values
    current_layer_softmax = F.softmax(current_layer, dim = -1)
    final_layer_softmax = F.softmax(final_layer, dim = -1)

    #### Calculate the average distribution 
    M = 0.5 * (current_layer_softmax + final_layer_softmax)

    ##### calculate the log softmax for kl divergence
    log_softmax_current_layer = F.log_softmax(current_layer, dim = -1)
    log_softmax_final_layer = F.log_softmax(final_layer, dim = -1)

    #### Calculate kl divergence
    kl_div1 = F.kl_div(log_softmax_current_layer, M, reduction = 'none').mean(-1)
    kl_div2 = F.kl_div(log_softmax_final_layer, M, reduction = 'none').mean(-1)

    ### JS divergence
    jsd = 0.5 * (kl_div1 + kl_div2)
    return jsd.item() 
...
def get_hstates_prediction(hstate, model):
    ...
    return predicted_tokens, logits.cpu().detach()
...
def make_layer_df(df_dict,
    ...
    # print(final_layer_prob.shape, prob_dist.shape)
    jsd_final_layer = JSD(current_layer = layer_logits,
                          final_layer= final_layer_logits)
    
    # print(layer_num, jsd_final_layer)
    prob_dist = F.softmax(layer_logits, dim=-1)
    entropy_dist = entropy(prob_dist)
    ...
    entropy_key = f'entropy_layer{layer_num}'
    df_dict[entropy_key] = [entropy_dist]
...
def get_token_layer_info_dict(summ_token_hstates,
    ...
    predicted_tokens, logits = get_hstates_prediction(summ_token_hstates, model)
    ...
    final_layer_logits= logits[-1]
    for layer_num in range(0, predicted_tokens.shape[0]):
            # prob_dist = logits[layer_num]
            layer_logits = logits[layer_num]
